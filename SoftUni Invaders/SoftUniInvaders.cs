...
class SoftUniInvaders
{
	...
	static void DisplayStartScreen()
	{
		...
		using (var file = new StreamReader(@"..\..\msg_startscreen.txt")) // print start screen
		{
			...
			{
				if (line < 7 || (line > 9 && line <= 16)) // draw stars
				{
					Console.ForegroundColor = ConsoleColor.Yellow;
				}
				else if (line <= 9)
				{
					Console.ForegroundColor = ConsoleColor.Cyan; // draw logo
				}
				else
				{
					Console.ForegroundColor = ConsoleColor.White; // draw controls and start text
				}
